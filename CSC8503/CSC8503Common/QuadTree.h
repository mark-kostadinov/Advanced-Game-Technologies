...
namespace NCL
{
	...
	namespace CSC8503
	{
			...
			void Insert(T & object, const Vector3 & objectPos, const Vector3 & objectSize, int depthLeft, int maxSize)
			{
				...
				{ // Currently a leaf node, can just expand
					...
					if ((int)contents.size() > maxSize && depthLeft > 0)
					{
						...
						{
							Split();
							// We need to re-insert the contents so far
		...
		protected:
			...
			void DebugDraw()
			{
				Vector3 topLeft		= Vector3(position.x, 0, position.y) + Vector3(-size.x, 0, size.y);
				Vector3 topRight	= Vector3(position.x, 0, position.y) + Vector3(size.x, 0, size.y);
				Vector3 bottomLeft	= Vector3(position.x, 0, position.y) + Vector3(-size.x, 0, -size.y);
				Vector3 bottomRight	= Vector3(position.x, 0, position.y) + Vector3(size.x, 0, -size.y);
