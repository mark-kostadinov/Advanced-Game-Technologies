...
bool RayBoxIntersection(const Ray&r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision)
{
	...
	for (int i = 0; i < 3; ++i) // Get the best 3 intersections
	{
		if (rayDir[i] > 0)
	...
	// backwards and forwards, so we can intersect with objects 'behind' us!
	if (tVals.GetMaxElement() < 0.0f)
		return false; // No backward rays!
	...
	for (int i = 0; i < 3; ++i)
	{
		if (intersection[i] + epsilon < boxMin[i] || intersection[i] - epsilon > boxMax[i])
			return false; // The best intersection doesn't touch the box!
	}
...
bool CollisionDetection::RaySphereIntersection(const Ray& ray, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision)
{
	...
	Vector3 direction = spherePosition - ray.GetPosition();
	// Then project the sphere's origin onto our ray direction vector
	float sphereProjection = Vector3::Dot(direction, ray.GetDirection());
	// Get closest point on ray line to sphere
	Vector3 point = ray.GetPosition() + (ray.GetDirection() * sphereProjection);
	...
	collision.rayDistance = sphereProjection - (sphereRadius * sNorm);
	collision.collidedAt = ray.GetPosition() + (ray.GetDirection() * collision.rayDistance);
...
Ray CollisionDetection::BuildRayFromCameraToArbitraryPosition(const Camera& cam, const Vector3& position)
{
	Vector3 direction = position;
	direction.Normalise();

	return Ray(cam.GetPosition(), direction);
}
	...
	const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
	...
	if (deltaLength < radii)
	{
		...
		Vector3 collisionPoint = collisionPoint = worldTransformA.GetWorldPosition() + (normal * (volumeA.GetRadius() -
			(penetration * 0.5f)));
	...
	const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo)
{
	...
	if (distance < volumeB.GetRadius()) // We are colliding
	{
